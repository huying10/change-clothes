...
def build_video_prompt(custom: str | None = None) -> str:
    """视频提示词（首帧 i2v）：锁定人脸、减少大幅运动以避免人脸漂移。"""
    prompt = (
        "以参考图为视频首帧，保持画面中人物的样貌完全不变。"
        "全程严格锁定人物的面部五官、脸型、发型与表情——每一帧的脸都必须与首帧是同一个人，"
        "不得变形、走样或漂移。"
        "人物面部始终大致朝向镜头，自然地缓步向前走动，"
        "不要大幅度转身、不要快速摆头，动作幅度小、舒缓流畅。"
        "人物与场景的光影、色调自然融合，竖屏 9:16，光线真实，画面稳定流畅。"
    )
    if custom:
        prompt += f" {custom.strip()}"
    return prompt
...
_IMG_IDENTITY = (
    "必须确保是与图1完全相同的同一个人：精确复刻图1人物的脸型轮廓、五官比例与位置、"
    "眼型、鼻型、嘴型、眉形、发型、肤色、体型胖瘦和性别，面部特征严格一致，"
    "不要美化、不要磨皮、不要改变长相"
)
...
def build_image_prompt(
    ...
    if labels:
        ...
        action = "保持图1中人物的服饰不变"
    action += f"，并自然地全身站立在图{scene_idx}的场景中。" if with_scene else "。"
    ...
    if with_scene:
        # 必须全身入镜（否则模型会裁成上半身）；同时强调保住人脸身份
        tail = (
            "完整展现人物全身，从头到脚都在画面内（包含上半身、下半身、腿部与鞋子），"
            "人物自然站立、全身居中入镜，不要裁剪到上半身。"
            "在此前提下严格保持图1人物的脸部、五官、表情与发型与原图一致，"
            "不要美化、不要改变脸型与五官比例。"
            "写实人像，人物与场景的光影、透视和色调自然融合，光线真实，高清。"
        )
    else:
        tail = _IMG_TAIL
