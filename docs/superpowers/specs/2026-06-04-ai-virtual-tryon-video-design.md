...
### 计费方式
Seedance 2.0 按**生成视频的秒数**计费，开发与本地运行本身不产生费用。

| 模式 | 单价 | 单次生成（10 秒）成本 |
|------|------|---------------------|
| Fast | 约 $0.081/秒 | 约 $0.81（≈ ¥6） |
| 标准 | 约 $0.1/秒 | 约 $1.0（≈ ¥7） |

> 1080P / 标准模式费用更高，480p + Fast 最省。

### Demo 阶段总成本估算
联调阶段需真实生成数十次以调效果，按 50 次 × ¥6 估算，
**整个 Demo 验证约 ¥300 以内**即可完成。实际取决于真实生成次数。
...
### 省钱开发策略（已落入设计）
1. **双模式可切换**：通过配置（`PROVIDER=mock|volcengine`）切换 Mock 与真实接口。
   - **Mock 模式**：返回预置的占位视频，零成本跑通"上传→任务→展示"全流程，先看交互效果。
   - **真实模式**：连火山引擎 Seedance 2.0，验证真实换装生成效果。
2. 真实验证优先用 **Fast 模式 + 480p**，效果满意后再偶尔跑标准 / 1080P 看高清。
3. 自动化测试一律走 Mock，不烧钱。

### 前置条件
火山引擎方舟账号需**实名 + 预充值**后方可调用（已开通，可直接连真实接口验证）。
